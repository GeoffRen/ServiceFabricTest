    ...
    public static class Program
    {
        public static void Main(string[] args)
        {
            try
            {
                ...
                ServiceEventSource.Current.Message("Registering Service : {0}", "UnattendedService");

                ServiceRuntime.RegisterServiceAsync("UnattendedServiceType",
                    context => new UnattendedService (context)).GetAwaiter().GetResult();

                ServiceEventSource.Current.Message("Registered Service : {0}", "UnattendedService");
